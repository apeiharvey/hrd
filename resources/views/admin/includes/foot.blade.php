  ...
  <script type="text/javascript" src="{{asset('plugins/bootstrap-select/js/bootstrap-select.js')}}"></script>
  <script type="text/javascript" src="{{asset('plugins/jquery-slimscroll/jquery.slimscroll.js')}}"></script>
  <script type="text/javascript" src="{{asset('plugins/node-waves/waves.js')}}"></script>
  <script type="text/javascript" src="{{asset('plugins/jquery-countto/jquery.countTo.js')}}"></script>
  <script type="text/javascript" src="{{asset('js/'.$view_path.'/admin.js')}}"></script>
  <script type="text/javascript" src="{{asset('js/'.$view_path.'/demo.js')}}"></script>
  <script type="text/javascript" src="{{asset('js/'.$view_path.'/base64.js')}}"></script>
  <script type="text/javascript" src="{{asset('js/'.$view_path.'/jquery.input-ip-address-control-1.0.min.js')}}"></script>
  ...
  <script type="text/javascript" src="{{asset('plugins/charts/loader.js')}}"></script>
  <script type="text/javascript" src="{{asset('plugins/owlcarousel/owl.carousel.min.js')}}"></script>
  <script type="text/javascript" src="{{asset('js/'.$view_path.'/sorttable.js')}}"></script> 
  <script type="text/javascript" src="{{asset('js/'.$view_path.'/sort.js')}}"></script> 
  ...
  $(document).ready(function(){
    ...
    $(document).on("click",".open-detail", function() {
      ...
      console.log('{{url("admin/applicants/detail/")}}'+filter+'/' + id+'-'+status);
      $.get('{{url("admin/applicants/detail/")}}'+filter+'/' + id+'-'+status, function( data ) {
        $(".modal-container").html(data);
        $("#myModal").css("display","block");
         tinymce.init({
            selector: "#content",theme: "modern",width: 680,height: 300,
            relative_urls: false,
            plugins: [
                 "advlist autolink link image lists charmap print preview hr anchor pagebreak",
                 "searchreplace wordcount visualblocks visualchars insertdatetime media nonbreaking",
            ...
            success:function(data, textStatus, jqXHR) 
                {
                    ...
                    if(data == 'success'){
                      ...
                      $(".modals-headers h2").text("Success");
                      $(".modals-body p").text("Email Has Been Delivered!");
                      $(".modals-body .p2").text("Don't Forget To Change the Applicant's Status!");
                      $(".modals-body p").css("color","#5cb85c");
                      $(".modals-headers").css("background-color","#5cb85c");
                      $(".modal").css('display','block');
                    }
                    if(data == 'fail'){
                      $("body").css("overflow","hidden");
                      $(".modals-headers h2").text("Failed");
                      $(".modals-body p").text(":(");
                      $(".modals-body .p2").text("Sorry, There is a Problem while Sending the Email");
                      $(".modals-body p").css("color","#e53935");
                      $(".modals-headers").css("background-color","#E91E63");
                      $(".modal").css('display','block');
                    }
                    if(data == 'empty'){
                      $("body").css("overflow","hidden");
                      $(".modals-headers h2").text("Failed");
                      $(".modals-body .p2").text("Please Fill In All the Data!");
                      $(".modals-body p").css("color","#e53935");
                      $(".modals-headers").css("background-color","#E91E63");
                      $(".modal").css('display','block');
                    }
