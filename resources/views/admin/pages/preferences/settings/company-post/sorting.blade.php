...
@section('content')
<form method="post" class="update" action="{{url($view_path.'/company-post/doSorting')}}">
    {!!csrf_field()!!}
                ...
                <div class=".col-xs-12 .col-sm-6 .col-md-8">
                    <div class="card">
                        <div class="header">
                            ...
                            <h2>
                                Sort Company Post Order
                                <small>You Can Sort the Company Post Order Here</small>
                            </h2>
                        </div>
                        <div class="body">
                                    ...
                                    <ul id="sortable">
                                        @foreach($companypost as $key)
                                            <li class="ui-state-default">
                                                <h6>{{$key->order}}</h6>
                                                <h5 data-toggle="tooltip" data-placement="bottom" title="{{$key->title}}" style="padding-top: 0px;">
                                                    @if(strlen($key->title)>25){{substr($key->title, 0, 25)."..."}}
                                                    @else {{$key->title}}
                                                    @endif
                                                </h5>
